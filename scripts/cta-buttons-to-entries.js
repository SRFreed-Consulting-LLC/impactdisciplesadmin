...
// Every archetype whose section-level buttons became a LIST (2026-08-31).
// The list archetypes are deliberately absent: their entries are the tiles
// and rows themselves, and each of those carries its own button already.
const CONVERTS = new Set(['heroBand', 'heroSplit', 'copyMedia', 'copyCentred', 'countdown']);

// Variants that existed only to say how many buttons there were, and the one
// variant that now covers both.
const VARIANT_MERGES = {
  'heroBand/standard': 'buttonList',
  'copyMedia/buttonList': 'image',
  'copyCentred/withButtons': 'plain'
};
...
function convert(block) {
  ...
  const merged = VARIANT_MERGES[block.type + '/' + block.variant];
  if (merged) {
    block.variant = merged;
  }
  return true;
}
...
async function main() {
  ...
  for (const doc of snap.docs) {
    ...
    for (const block of blocks) {
      ...
      if (convert(block)) {
        touched = true;
        console.log('  ' + doc.id.padEnd(26) + block.type.padEnd(13) + block.variant.padEnd(12)
  ...
  for (const key of Object.keys(seed)) {
    ...
    for (const block of blocks) {
      if (CONVERTS.has(block.type) && convert(block)) {
        seedTouched++;
      }
    }
  }
