...
export class AdminManagerComponent implements OnInit {
  ...
  // second, locally-duplicated list - this module has no per-tab role
  // gating (only the group itself, already filtered before the left nav
  // ever offers this route), so no further filtering is needed here.
  tabs = NAV_CONFIG.find((g) => g.id === 'admin-manager')!.items!;
