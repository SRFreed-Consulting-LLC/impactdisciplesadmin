...
import { collection, deleteDoc, doc, getDoc, getDocs, orderBy, query, setDoc } from '@angular/fire/firestore';
 ...
 * in the named 'impactdiscipleship-books' database - see
 * library-firestore.util.ts's own comment for why this MUST construct its
 * handle through that factory rather than injecting the shared Firestore
 * instance. One-shot reads (getTiers()), not the source app's live
 * `collectionData` listener, matching every other Library service ported so
 * far in this app.
 *
...
export class LibraryBulkDiscountTierService {
  ...
  async getTiers(): Promise<BulkDiscountTier[]> {
    const snap = await getDocs(
      query(collection(libraryFirestore(this.app), 'bulkDiscountTiers'), orderBy('numberOfBooks')),
    );
  ...
  async createTier(numberOfBooks: number, percentOff: number): Promise<void> {
    const ref = doc(libraryFirestore(this.app), 'bulkDiscountTiers', String(numberOfBooks));
  ...
  async deleteTier(numberOfBooks: number): Promise<void> {
    await deleteDoc(doc(libraryFirestore(this.app), 'bulkDiscountTiers', String(numberOfBooks)));
