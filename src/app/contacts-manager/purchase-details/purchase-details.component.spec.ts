...
describe('PurchaseDetailsComponent (characterization, pre-split)', () => {
  // The timeline is the densest derivation on this screen and none of it is
  // visible from the template.
  describe('timeline', () => {
  ...
  describe('refund history', () => {
    it('passes recorded refunds through unchanged', () => {
      ...
      const { component } = makeComponent(aPurchase({ refunds } as Partial<CheckoutForm>));
    ...
    it('is empty for an order that was never refunded', () => {
      const { component } = makeComponent(aPurchase());
      expect(component.refundHistory()).toEqual([]);
    });
  });
  ...
  describe('workflow actions', () => {
    ...
    it('markShippedViaAmazon moves it onto the Amazon path', async () => {
      ...
      // And the timeline should now follow that branch.
      expect(component.timeline.some((n) => n.step.label === 'Shipped via Amazon')).toBeTrue();
    });
